...
{
    for (int i = 0; i < World::GetInstance()->GetXSize(); i++)
    {
        for (int j = 0; j < World::GetInstance()->GetYSize(); j++)
        {
            SDL_Texture *texture = LoadImage(World::GetInstance()->GetBlockData(i, j)->GetTexturePath(), World::GetInstance()->GetBlockData(i, j)->isTransparent());
            ...
            SDL_Rect drawRect{i * BlocksPixelSize, j * BlocksPixelSize, (*imageWidth) * widthMagnification, (*imageHeight) * heightMagnification};
